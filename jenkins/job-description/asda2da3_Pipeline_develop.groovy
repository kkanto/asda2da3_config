...
def job_config = [
        job: [
                name: "asda2da3_Pipeline_develop",
                agent: "maven"
        ],
        git: [
                branch: "develop"
...
def customLib = library identifier: 'asda2da3_JSL@develop', retriever: modernSCM(
  [\$class: 'GitSCMSource',
   remote: 'https://github.developer.allianz.io/kkanto/asda2da3_lib.git',
   credentialsId: 'git-token-credentials']) 
...
def jslCustom     = customLib.de.allianz.asda2da3.new()
    ...
    stages {
        ...
        stage('Checkout') {
            steps {
                echo "checkout"
                script {
                   jslGit.checkout( config, "kkanto", "asda2da3", job_config.git.branch)
               }
            }    
        }
        stage('Build') {
...
def job = pipelineJob("asda2da3_Pipeline_develop");

job.with {

    authenticationToken('3mxHgxpfGVjpyRtxtYgf2cx6EdJSBQzf')
